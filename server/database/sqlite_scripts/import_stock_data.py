    ...
    for i, row in historical_data.iterrows():
        ...
        try:
            cursor.execute("""INSERT INTO stock_data(
                                ...
                                dividends,
                                stock_splits) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""", (isin, i.strftime('%Y-%m-%d'), row['Open'], row['High'], row['Low'], row['Close'], row['Volume'], row['Dividends'], row['Stock Splits']))
            connection.commit()
            if wrote is False:
                print(f"Daten eingegeben für {name}, isin {isin}")
                wrote = True
        except:
            print(f"Fehler Daten von {name} isin: {isin}"
                  f"konnte nicht eingetragen werden")
